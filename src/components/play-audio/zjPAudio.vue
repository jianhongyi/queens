...
<template>
  <div class="queens-play-audio">
    <div class="playAudio" :class="{ playing: playing }">
      <span class="play-btn" @click.stop="play">
        <template v-if="!canplay">
          <svg viewBox="25 25 50 50" class="queens-loading__circular">
    ...
    <audio ref="audio" preload="auto" class="audio" :src="url" />
  </div>
</template>
<script>
...
.queens-play-audio {
    ...
    .duration {
      ...
      &-bar {
        ...
        &:after{
          ...
          width: 100%;
          height: 4px;
          background-color: rgba(77, 141, 255, .7);
          border-radius: 3px;
        }
      }
      &-toggle {
  ...
  .queens-loading__circular{
    display: block;
    width: 70%;
    height: 70%;
    margin: 15% auto 0;
    circle {
      animation: queens-circular 1.5s ease-in-out infinite;
      stroke: #4D8DFF;
      stroke-width: 6;
