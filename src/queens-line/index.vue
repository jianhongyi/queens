<!-- 连线题题组件 -->
<template>
  <div :style="wrapperStyle" class="ques-liner-wrapper">
    <div :style="mainAreaStyle" :class="['main-area', mainAreaClass]">
      <div ref="scrollCard" class="scrollCard">
        <div class="scroll-card-inner">
          ...
          <queens-liner ref="quesLiner" :key="quesData.questionId" :options="quesData.options || []" :answers="quesData.answers || []" :user-answer="quesData.userAnswer || {}" :template-type="quesData.templateType || ''" v-bind="$attrs" />
        </div>
        <slot name="footer" />
        <div class="pad" />
      </div>
  ...
  methods: {
    checkAnswer() {
      return new Promise((resolve, reject) => {
        try {
          setTimeout(() => {
            this.$refs.quesLiner?.checkAnswer((userCheckedAnswers, trueAnswers, answerResult) => {
              const returnObj = {
                userCheckedAnswers,
                trueAnswers,
                answerResult
